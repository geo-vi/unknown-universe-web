...
class System
{
    ...
    public function Login($username, $password)
    {
        ...
        if ($username == $UserData['USERNAME'] && password_verify($password, $UserData['PW_HASH'])) {
            ...
            $this->mysql->QUERY('UPDATE users SET IP = ? WHERE SESSION_ID = ?',
                                [
                                    $ip,
                                    session_id(),
                                ]
    ...
    public function sendRecovery($USERNAME, $EMAIL)
    {
        ...
        $QUERY = $this->mysql->QUERY('INSERT INTO server_recovery (USER_ID, ACTIVATION_CODE, SEND_TO, SEND_DATE, TIMEOUT)
                  VALUES (?,?,?,?,?)',
                                     [
                                         $User[0]['USER_ID'],
                                         $CODE,
    ...
    public function isLoggedIn()
    {
        if (isset($_SESSION["USER_ID"]) && $this->isValid($_SESSION["USER_ID"])) {
            ...
            if (count($user) == 1) {
                ...
                if ( !$this->User->__get('VERFIED') && NEED_EMAIL_VERIFY) {
                    $this->sendEmailVerification($this->User->__get('USER_ID'), $this->User->__get('EMAIL'));
                    $this->error_handler->throwError(ErrorID::EMAIL_MISSING);
                }

                //LOG ALL USER ACTIONS
                if ($this->routing->request != '/core/ajax/ajax.php') {
                    $this->logging->addLog($this->User->__get('USER_ID'),
                                           $this->User->__get('PLAYER_ID'),
                                           $this->User->__get('SERVER_DB'),
                ...
                if ($this->User->__get('LAST_SERVER') == null) {
                    ...
                    if ($this->getCurrentServer(true) != $this->User->__get('LAST_SERVER')) {
                        if ($this->Server) {
                            ...
                            $this->Server = $this->getServer($this->User->__get('LAST_SERVER'));
                            $this->redirectToServer();
                        }
                    }
    ...
    public function sendEmailVerification($USER_ID, $EMAIL)
    {
        ...
        $QUERY = $this->mysql->QUERY('INSERT INTO server_verfiy (USER_ID, ACTIVATION_CODE, SEND_TO, SEND_DATE, TIMEOUT)
                  VALUES (?,?,?,?,?)',
                                     [
                                         $USER_ID,
                                         $CODE,
