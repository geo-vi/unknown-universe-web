...
class EquipmentHandler extends AbstractHandler
{
    ...
    public function exec_switch_hangar(){
        ...
        if($System->User->Hangars->setHangar($this->params['HANGAR_ID'])){
            die(json_encode(['error' => false, "NEW_SHIP" => $System->User->getShipImage(), "HANGAR_ID" => $System->User->Hangars->CURRENT_HANGAR->ID]));
        }else{
        ...
        if($TO == 'equip'){
            ...
            switch ($DISPLAY){
                case 'ship':
                    ...
                    foreach ($ITEMS as $ITEM){
                        ...
                        if(isset($SLOT_TYPES[strtoupper($ITEM->CATEGORY)]['ITEMS'][$ITEM->LOOT_ID]) ||
                        ...
                        if($SLOT_TYPES[strtoupper($ITEM->CATEGORY)]['MAX'] > $SLOT_TYPES[strtoupper($ITEM->CATEGORY)]['IN_USE']){
                            if($ITEM->moveToEquipment($System->User->Hangars->CURRENT_HANGAR->ID, $CONFIG, ['target' =>$DISPLAY])){
                                $SLOT_TYPES[strtoupper($ITEM->CATEGORY)]['IN_USE']++;
                            }else{
                ...
                case 'drone':
                    ...
                    foreach ($ITEMS as $ITEM){
                        ...
                        if($ITEM_OBJ->CATEGORY == 'extras' || $ITEM_OBJ->CATEGORY == 'heavy' || $ITEM_OBJ->ATTRIBUTES['SPEED'] != null) continue;
                        ...
                        if(isset($ITEM['DRONE_ID'])){
                            if(isset($DRONES[$ITEM['DRONE_ID']])){
                                if( ($DRONES_USAGE[$ITEM['DRONE_ID']] < $DRONES[$ITEM['DRONE_ID']]['SLOTS']) || ($ITEM['CATEGORY'] == 'drone_design' && $DRONES_DESIGN[$ITEM['DRONE_ID']] < 1)){
									if($ITEM_OBJ->moveToEquipment($System->User->Hangars->CURRENT_HANGAR->ID, $CONFIG, ['target' =>$DISPLAY, 'droneID' => $ITEM['DRONE_ID']])){
                                        if($ITEM['CATEGORY'] == 'drone_design'){
                                            $DRONES_DESIGN[$ITEM['DRONE_ID']]++;
                                        }else{
                                            $DRONES_USAGE[$ITEM['DRONE_ID']]++;
                                ...
                                die(json_encode(['error' => true, "error_msg" => "Drone doesnt exists! You really want a ban for that?"]));
                            }
                        }else{
                            foreach ($DRONES as $DRONE){
                                if($DRONES_USAGE[$DRONE['ID']] < $DRONE['SLOTS']){
                                    if($ITEM_OBJ->moveToEquipment($System->User->Hangars->CURRENT_HANGAR->ID, $CONFIG, ['target' =>$DISPLAY, 'droneID' => $DRONE['ID']])){
            ...
            foreach ($ITEMS as $ITEM){
                ...
                if($ITEM){
                    ...
                    die(json_encode(['error' => true, "error_msg" => "The requested ITEM to move doesnt exists in your Inventory... try that again and get banned!"]));
                }
            }
