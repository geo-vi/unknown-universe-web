...
class User
{
    ...
    function __construct($mysql, $AccountData)
    {
        ...
        $this->SERVER_DB = $this->mysql->QUERY("SELECT DB_NAME FROM server_infos WHERE SHORTCUT = ?", array($this->LAST_SERVER))[0]['DB_NAME'];
        ...
        $PlayerData = $this->mysql->QUERY("SELECT SESSION_ID FROM player_data WHERE player_data.USER_ID = ?", array($this->USER_ID));
    ...
    public function refresh(){
        ...
        //REFRESH player_data
        $PlayerData = $this->mysql->QUERY("SELECT * FROM player_data WHERE player_data.USER_ID = ?", array($this->USER_ID));
        $this->PlayerData = $PlayerData[0];

        $this->mysql->QUERY("UPDATE player_data SET SESSION_ID = ? WHERE  USER_ID = ?", array($this->SESSION_ID, $this->USER_ID));
    ...
    public function getShipImage($Design  = ""){
        ...
        $loot_id = $this->mysql->QUERY("SELECT ship_lootid FROM server_ships WHERE ship_id = ?", array($Design))[0]['ship_lootid'];
    ...
    public function setFaction($FactionID){
        if($this->FACTION_ID == 0){
            if($FactionID === 1 || $FactionID === 2 || $FactionID === 3){
                return $this->mysql->QUERY('UPDATE player_data SET FACTION_ID = ? WHERE USER_ID = ? AND PLAYER_ID = ?', array($FactionID, $this->USER_ID, $this->PLAYER_ID));
            }else{
                return false;
            }
        }else {
    ...
    public function hasPremium(){
        ...
        if($PremiumDate < $Today){
            if($Premium){
                $this->mysql->QUERY('UPDATE player_data SET PREMIUM = 0 WHERE USER_ID = ? AND PLAYER_ID = ?', array($this->USER_ID, $this->PLAYER_ID));
                return false;
            }else{
                return false;
            }
    ...
    public function hasPet(){
        $pet = $this->mysql->QUERY('SELECT * FROM player_pet WHERE PLAYER_ID = ? AND USER_ID = ?', array($this->PLAYER_ID, $this->USER_ID));
    ...
    public function hasDrones($returnCount = false,$splitTypes = false){
        if($returnCount && $splitTypes){
            $droneFlax = $this->mysql->QUERY("SELECT ID FROM player_drones WHERE USER_ID = ? AND PLAYER_ID = ? AND DRONE_TYPE = 0",array($this->USER_ID,$this->PLAYER_ID));
            $droneIris = $this->mysql->QUERY("SELECT ID FROM player_drones WHERE USER_ID = ? AND PLAYER_ID = ? AND DRONE_TYPE = 1",array($this->USER_ID,$this->PLAYER_ID));
            $droneApis = $this->mysql->QUERY("SELECT ID FROM player_drones WHERE USER_ID = ? AND PLAYER_ID = ? AND DRONE_TYPE = 2",array($this->USER_ID,$this->PLAYER_ID));
            $droneZeus = $this->mysql->QUERY("SELECT ID FROM player_drones WHERE USER_ID = ? AND PLAYER_ID = ? AND DRONE_TYPE = 3",array($this->USER_ID,$this->PLAYER_ID));
            $result = array(
            ...
            $droneResult = $this->mysql->QUERY("SELECT ID, DRONE_TYPE FROM player_drones WHERE USER_ID = ? AND PLAYER_ID = ?",array($this->USER_ID,$this->PLAYER_ID));
        }
    ...
    public function getLevelProgress(){
        $ExpNeed = $this->mysql->QUERY('SELECT * FROM server_levels_player WHERE ID = ?', array($this->LVL + 1))[0]['EXP'];
    ...
    public function getRankName(){
        $RankName = $this->mysql->QUERY('SELECT RANK_NAME FROM server_ranks WHERE ID = ?', array($this->RANK))[0]['RANK_NAME'];
        return $RankName;
    }
    ...
    public function create(){
        if($this->USER_ID == $_SESSION["USER_ID"]){
            ...
            if($this->mysql->QUERY(
                ...
                $default_ship = $this->mysql->QUERY("SELECT * FROM server_ships WHERE ship_id =  ?",array($server_config["DEFAULT_SHIP"]))[0];
                ...
                foreach($items->items as $item){
                    $item_info = $this->mysql->QUERY("SELECT * FROM server_items WHERE ID = ?",array($item->ID))[0];
                    for($i =1; $i <= $item->Q ;$i++){
                        $this->mysql->QUERY(
                ...
                //GIVE DEFAULT AMMO
                $this->mysql->QUERY("INSERT INTO player_ammo (USER_ID,PLAYER_ID) VALUES(?,?)",array($this->USER_ID, $this->PLAYER_ID));
                ...
                $this->mysql->QUERY("INSERT INTO player_extra_data (USER_ID,PLAYER_ID) VALUES(?,?)",array($this->USER_ID, $this->PLAYER_ID));

                //INSERT player_ship_config
                $hangarID = $this->mysql->QUERY("SELECT ID FROM player_hangar WHERE USER_ID = ? AND PLAYER_ID = ? AND ACTIVE = 1",array($this->USER_ID, $this->PLAYER_ID))[0];
                $this->mysql->QUERY("INSERT INTO player_ship_config (USER_ID,PLAYER_ID,HANGAR_ID) VALUES(?,?,?)",array($this->USER_ID, $this->PLAYER_ID, $hangarID["ID"]));

                //INSERT player_queststats
                $this->mysql->QUERY("INSERT INTO player_queststats (USER_ID,PLAYER_ID) VALUES(?,?)",array($this->USER_ID, $this->PLAYER_ID));

                //INSERT player_skill_tree
                $this->mysql->QUERY("INSERT INTO player_skill_tree(USER_ID,PLAYER_ID) VALUES(?,?)",array($this->USER_ID, $this->PLAYER_ID));
    ...
    public function fixDB(){
        ...
        foreach ($server_items as $item){
            $this->mysql->QUERY('UPDATE player_equipment SET ITEM_TYPE = ? WHERE ITEM_ID = ?',array($item['TYPE'], $item['ID']));
        }
    }

    public function getTitle()
    {
		if ($this->TITLE_ID == 0) return "";
        $title = $this->mysql->QUERY('SELECT TITLE_NAME FROM server_titles WHERE ID = ?', array($this->TITLE_ID))[0]['TITLE_NAME'];
        return $title;
    }

    public function getTitleColor(){
		if ($this->TITLE_ID == 0) return "#fff";
		$titleColor = $this->mysql->QUERY('SELECT TITLE_COLOR_HEX FROM server_titles WHERE ID = ?', array($this->TITLE_ID))[0]['TITLE_COLOR_HEX'];
        return $titleColor;
    }
