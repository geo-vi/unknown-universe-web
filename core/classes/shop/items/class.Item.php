    ...
    public function buy($UserID, $PlayerID, $Amount = 1)
    {
        ...
        if(strpos($this->LOOT_ID, 'resource_') !== false){
            ...
            if(strpos($this->LOOT_ID, 'resource_booty-key') !== false){
                ...
                return $this->mysql->QUERY('UPDATE player_extra_data SET BOOTY_KEYS = ? WHERE USER_ID = ? AND PLAYER_ID = ?',array($BOOTY_KEYS, $UserID, $PlayerID));
            }
            else if(strpos($this->LOOT_ID, 'resource_logfile') !== false){
                $LOGFILES = (int) $System->User->LOGFILES;
                $LOGFILES += $Amount;
                return $this->mysql->QUERY('UPDATE player_extra_data SET LOGFILES = ? WHERE USER_ID = ? AND PLAYER_ID = ?',array($LOGFILES, $UserID, $PlayerID));
            }

        }
        else{
            for($Success = 0; $Success < $Amount;){
                if($this->mysql->QUERY("INSERT INTO player_equipment (USER_ID, PLAYER_ID, ITEM_ID, ITEM_TYPE, ITEM_AMOUNT) VALUES(?,?,?,?,?)", array($UserID, $PlayerID, $this->ID, $ITEM_DATA['TYPE'], $ITEM_DATA['USES']))){
                    $Success++;
                }
            }
