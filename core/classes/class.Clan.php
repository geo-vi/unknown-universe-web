...
class Clan
{
    ...
    public function searchClan(string $string){
        $string = '%'.$string.'%';
        return $this->mysql->QUERY("SELECT * FROM server_clans WHERE DESCRIPTION LIKE ? OR NAME LIKE ? OR TAG LIKE ?", [$string, $string, $string]);
    }

    private function getMyNewClan($tag){
        $clan = $this->mysql->QUERY("SELECT * FROM server_clans WHERE TAG = ?", array($tag));
        return $clan;
    }
    ...
    private function regClan($args){
		...
		$New = $this->mysql->QUERY("INSERT INTO server_clans (NAME, LEADER,TAG,MEMBERS,FACTION,IS_ACCEPTING) VALUES(?,?,?,?,?,?)",array($args['clan_name'],$this->User->USER_ID,$args['clan_tag'],$this->User->USER_ID,$FACTION_ID,$n));
    ...
    public function foundClan($args){
		...
		else if (strlen($clanName) < 5 || strlen($clanName) > 20)
			return "Invalid clan name";
		
		$result = $this->mysql->QUERY("SELECT * FROM server_clans WHERE TAG = ? || NAME = ? || LEADER = ?", array($args['clan_tag'], $args['clan_name'], $this->User->USER_ID));
		if (empty($result)) {
