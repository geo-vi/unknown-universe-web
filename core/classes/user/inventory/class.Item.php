...
class item
{
    ...
    function __construct($ItemData, $mysql)
    {
        ...
        $this->CONFIGS = $this->mysql->QUERY('SELECT ON_CONFIG_1, ON_CONFIG_2, ON_DRONE_ID_1, ON_DRONE_ID_2, ON_PET_1, ON_PET_2 
                                      FROM player_equipment WHERE ID = ?', array($this->ID))[0];
    }
    ...
    public function moveToEquipment($HangarID, $Config, $To){
        if(!isset($To['target'])) return false;
        if($this->isInUse($HangarID, $Config)) return false;
        ...
        switch ($To['target']){
            case 'ship':
                ...
                break;
            case 'drone':
                if(!isset($To['droneID'])) return false;
        ...
        return $this->mysql->QUERY('UPDATE player_equipment SET '.$Location.' = ? WHERE ID = ?',
            array(json_encode($Config), $this->ID));
    }
    ...
    public function delete(){
        return $this->mysql->QUERY('DELETE FROM player_equipment WHERE ID = ?', array($this->ID));
    }
}
