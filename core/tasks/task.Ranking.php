...
$server_list = $System->mysql->QUERY('SELECT * FROM server_infos',array());
...
foreach ($server_list as $server){
    ...
    $player_list = $server_con->QUERY('SELECT * FROM player_data WHERE RANK != 21',array());

    foreach ($player_list as $player){
        $player_extra_data = $server_con->QUERY('SELECT * FROM player_extra_data WHERE USER_ID = ?  AND PLAYER_ID = ?',array($player['USER_ID'],$player['PLAYER_ID']))[0];
        $player_ship = $server_con->QUERY('SELECT SHIP_ID FROM player_hangar WHERE USER_ID = ? AND PLAYER_ID = ? AND ACTIVE = 1',array($player['USER_ID'],$player['PLAYER_ID']))[0];
        $server_npcs_data = $server_con->QUERY('SELECT ship_id, ship_hp FROM server_ships',array());
        ...
        $server_con->QUERY('UPDATE player_data SET RANK_POINTS = ? WHERE USER_ID = ? AND PLAYER_ID = ?',array($RankPoints,$player['USER_ID'],$player['PLAYER_ID']));
    }

    //Creating Ranks for each Company
    for($i = 1;$i <= 3;$i++){
        $player_list = $server_con->QUERY('SELECT * FROM player_data WHERE FACTION_ID = ? AND RANK != 21 ORDER BY RANK_POINTS DESC',array($i));
        ...
        foreach ($player_list as $player){
            ...
            $server_con->QUERY('UPDATE player_data SET RANK = ? WHERE USER_ID = ? AND PLAYER_ID = ?',array($Rank,$player['USER_ID'],$player['PLAYER_ID']));
        }
    }

    //Creating Top Rankings
    $player_list = $server_con->QUERY('SELECT * FROM player_data WHERE  RANK != 21 ORDER BY RANK_POINTS DESC',array());

    foreach ($player_list as $pos => $player){
        $ranking = $pos+1;
        $server_con->QUERY('UPDATE player_data SET RANKING = ? WHERE USER_ID = ? AND PLAYER_ID = ?',array($ranking,$player['USER_ID'],$player['PLAYER_ID']));
    }
}
