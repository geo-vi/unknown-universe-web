...
function sendCoreRequest(handler, action, params) {
  ...
  $.ajax({
    ...
    error: function error(errorData, _, errorThrown) {
      if (errorCallback !== null) errorCallback(errorData);

      if (errorData !== null) {
        swal(errorThrown + '!', errorData.responseJSON.message || errorThrown, 'error');
      }
    }
  });
}

function sendQuietRequest(handler, action, params) {
  var successCallback = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : null;
  var errorCallback = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : null;
  var data = {
    'handler': handler,
...
__webpack_require__(/*! ./clan */ "./resources/js/clan.js");

__webpack_require__(/*! ./equipment */ "./resources/js/equipment.js");

__webpack_require__(/*! ./galaxygate */ "./resources/js/galaxygate.js");
...
__webpack_require__(/*! ./skylab */ "./resources/js/skylab.js");
...
/*#__PURE__*/
function () {
  function clan(USER_ID, PLAYER_ID) {
  ...
  _createClass(clan, null, [{
      ...
      if (data === null) {
        ...
        if (CLANS.length === 0) {
          ...
          CLANS.forEach(function (clanEntry) {
            var CLAN_MEMBERS = clanEntry['MEMBERS'];
            var FACTION = 'ALL';
            ...
            $('.clan-bottom-list-container .table > tbody:last-child').append('<tr data-clan-id="' + clanEntry['ID'] + '"><td>' + clanEntry['TAG'] + '</td>\n' + '                            <td>' + clanEntry['NAME'] + '</td>\n' + '                            <td>' + clanEntry['DESCRIPTION'] + '</td>\n' + '                            <td>' + CLAN_MEMBERS.length + '/50</td>\n' + '                            <td>' + FACTION + '</td>\n' + '                            <td><button type="button" class="requestJoinButton btn btn-primary">Join</button>\n</td></tr>');
          });
          var JOIN_REQUESTS = data['JOIN_REQUESTS'];
          JOIN_REQUESTS.forEach(function (joinReq) {
            var requstedClanRow = $(".clan-bottom-list-container .table > tbody > tr[data-clan-id='" + joinReq['CLAN_ID'] + "']");
            var button = requstedClanRow.find('.requestJoinButton');
    ...
    value: function renderInternal() {
      ...
      if (data === null) {
        ...
        MEMBERS.forEach(function (clanMember) {
          var isMe = clanMember['USER_ID'] == clan.USER_ID;
          var rank = isMe ? '<button type="button" class="leaveButton btn btn-danger">Leave</button>' : clanMember['RANK_NAME'];
          var category = isMe ? 'self' : 'member';
        ...
        var REQUESTS = data['JOIN_REQUESTS'];
        ...
        var FACTION_ELEMENT = $('#faction');
        var FACTION = FACTION_ELEMENT.data('value');
        var FACTION_NAME = clan.getFaction(FACTION);
        FACTION_ELEMENT.text(FACTION_NAME); // $('#members-list ul:last-child').append('<li><a href="#" data-user-id="1">general_Rejection</a></li>');
    ...
    value: function clanSearch() {
      ...
      if (data === null) {
        ...
        data.forEach(function (clanEntry) {
          var CLAN_MEMBERS = [];
          var members = JSON.parse(clanEntry['MEMBERS']);
          CLAN_MEMBERS.push(members);
          var FACTION = clan.getFaction(clanEntry['FACTION']);
          $('.clan-bottom-list-container .table > tbody:last-child').append('<tr data-clan-id="' + clanEntry['ID'] + '"><td>' + clanEntry['TAG'] + '</td>\n' + '                            <td>' + clanEntry['NAME'] + '</td>\n' + '                            <td>' + clanEntry['DESCRIPTION'] + '</td>\n' + '                            <td>' + CLAN_MEMBERS.length + '/50</td>\n' + '                            <td>' + FACTION + '</td></tr>');
        });
      }
    }
    ...
    value: function cancelButtonClicked() {
      var data = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
      var parent = $(".cancelButton").parent().parent();
    ...
    key: "acceptButtonClicked",
    value: function acceptButtonClicked() {
      var data = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
    ...
    key: "joinButtonClicked",
    value: function joinButtonClicked() {
      var data = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
      ...
      if (data === null) {
        ...
        var clanId = parent.data('clan-id');
        var params = {
          'CLAN_ID': clanId
        };
    ...
    key: "kickButtonClicked",
    value: function kickButtonClicked() {
      var data = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
    ...
    key: "makeLeaderClicked",
    value: function makeLeaderClicked() {
      var data = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
    ...
    key: "sendRequest",
    value: function sendRequest(callback, action) {
...
/*#__PURE__*/
function () {
  function equipment(USER_ID, PLAYER_ID, SERVER_IP) {
  ...
  _createClass(equipment, null, [{
    ...
    value: function render() {
      ...
      if (equipment.data === null && data === null) {
        ...
        equipment.preRender();
        var SLOTS = this.data.SHIP.SLOTS["CONFIG_" + equipment.config],
        ...
        var PETS = this.data.PET.length !== 0 ? this.data.PET.SLOTS["CONFIG_" + equipment.config] : []; //CREATE DESIGN SWITCH

        $('#design-select').empty();

        for (var DESIGN in DESIGNS) {
          var optionItem = $('<option>').attr('value', DESIGN).text(DESIGNS[DESIGN].NAME);
        ...
        $('#filter-select').empty();
        var optionItem1 = $('<option>').attr('value', '0').text('DEFAULT'),
        ...
        for (var ITEM in ITEMS_COLLACTION[equipment.display]) {
          var item = ITEMS_COLLACTION[equipment.display][ITEM];
        ...
        if (equipment.display === "ship") {
          ...
          for (var SLOT_TYPE in SLOTS) {
            var equipmentSlots = $("#config-" + equipment.config + " > div." + equipment.display + "-equipment-container > ." + equipment.display + "-equipment-slots." + SLOT_TYPE.toLowerCase() + "-slots"); //CREATE EQUIPMENT SLOTS

            for (var i = 0; i < parseInt(SLOTS[SLOT_TYPE]); i++) {
              var equipmentSlot = $('<div>'),
                  levelText = $('<span>'),
                  _ITEM = null,
                  CURRENT_ITEMS = SORTED_ITEMS[SLOT_TYPE.toLowerCase()]; //CHECKS FOR EQUIPED ITEMS

              if (i < CURRENT_ITEMS.length) {
                _ITEM = CURRENT_ITEMS[i];

                if (_ITEM.LVL >= 1 && _ITEM.CATEGORY !== 'extra' && _ITEM.ATTRIBUTES.SPEED === null) {
                  $(levelText).text(_ITEM.LVL);
                }

                $(equipmentSlot).attr("data-type", _ITEM.CATEGORY.toLowerCase()).attr("data-item-id", _ITEM.ID).attr("data-item-name", _ITEM.ITEM_NAME.toLowerCase()).addClass("equiped-item").append($(levelText));
              } else {
          ...
          //RENDER SHIP / PET SLOTS
          for (var _SLOT_TYPE in PETS) {
            var _equipmentSlots = $("#config-" + equipment.config + " > div.pet-equipment-container > .pet-equipment-slots." + _SLOT_TYPE.toLowerCase() + "-slots"); //CREATE EQUIPMENT SLOTS


            for (var _i = 0; _i < parseInt(PETS[_SLOT_TYPE]); _i++) {
              var _equipmentSlot = $('<div>'),
                  _levelText = $('<span>'),
                  _ITEM2 = null,
                  _CURRENT_ITEMS = SORTED_ITEMS[_SLOT_TYPE.toLowerCase()]; //CHECKS FOR EQUIPPED ITEMS


              if (_i < _CURRENT_ITEMS.length) {
                _ITEM2 = _CURRENT_ITEMS[_i];

                if (_ITEM2.LVL >= 1 && _ITEM2.CATEGORY !== 'extra') {
                  $(_levelText).text(_ITEM2.LVL);
                }

                $(_equipmentSlot).attr("data-type", _ITEM2.CATEGORY.toLowerCase()).attr("data-item-id", _ITEM2.ID).attr("data-item-name", _ITEM2.ITEM_NAME.toLowerCase()).addClass("equiped-item").append($(_levelText));
              } else {
                $(_equipmentSlot).addClass("equipment-slot");
              } //APPEND SLOT TO EQUIPMENT


              $(_equipmentSlots).append(_equipmentSlot);
            }
          } //RENDER SHIP INFO
          ...
          var SORTED_DRONES = []; //SORT DRONES BY DRONE_ID

          for (var DRONE in DRONES) {
            var CURRENT_DRONE = DRONES[DRONE];
          ...
          for (var ITEM_TYPE in SORTED_ITEMS) {
            if (ITEM_TYPE === "laser" || ITEM_TYPE === "generator" || ITEM_TYPE === "drone_design") {
              var _CURRENT_ITEMS2 = SORTED_ITEMS[ITEM_TYPE];

              for (var _ITEM3 in _CURRENT_ITEMS2) {
                _ITEM3 = _CURRENT_ITEMS2[_ITEM3];

                if (ITEM_TYPE === "drone_design") {
                  SORTED_DRONES[_ITEM3.DRONE_ID]["DRONE_DESIGN"] = _ITEM3;
                } else {
                  if (SORTED_DRONES[_ITEM3.DRONE_ID] !== undefined) {
                    SORTED_DRONES[_ITEM3.DRONE_ID]["ITEMS"].push(_ITEM3);
                  }
                }
              }
            }
          ...
          for (var _DRONE in SORTED_DRONES) {
            _DRONE = SORTED_DRONES[_DRONE];
            if (_DRONE.LEVEL == 6) _DRONE.LEVEL--; //PREPARE HTML

            var droneContainer = $("<div>").addClass("drone-container").attr("data-drone-id", _DRONE.ID),
                droneImageDiv = $('<div>').addClass("drone-image"),
                droneImageUrl = "./resources/images/items/drone/" + _DRONE.NAME.toLowerCase() + "-" + _DRONE.LEVEL + "_top.png",
                droneImage = $('<img>').attr("src", droneImageUrl),
                droneULevel = $('<div>').addClass("drone-upgrade-level").text(_DRONE.UPGRADE_LEVEL),
                droneLevel = $('<div>').addClass("drone-level").text("Level: " + parseInt(_DRONE.LEVEL)),
                droneDesignName = $("<div>").addClass("drone-design-name").text("Design"),
                droneEffect = $('<div>').addClass("drone-effect").text("Experience: " + _DRONE.EXPERIENCE),
                droneDamage = $('<div>').addClass("drone-damage").text("Damage: " + _DRONE.DAMAGE),
                droneEquipmentSlots = $('<div>').addClass("drone-equipment-slots laser-slots generator-slots").data('category', 'laser|generator'),
                droneDesignSlots = $('<div>').addClass("drone-equipment-slots drone_design-slots").data('category', 'drone_design'); //PUT TOGETHER DRONE IMAGE

            $(droneImageDiv).append(droneImage);
            $(droneContainer).append(droneImageDiv); //CREATE EQUIPMENT SLOTS

            for (var slots = 0; slots < parseInt(_DRONE.SLOTS); slots++) {
              var _equipmentSlot2 = $('<div>'),
                  _levelText2 = $('<span>');

              if (slots < _DRONE.ITEMS.length) {
                var _ITEM4 = _DRONE.ITEMS[slots];

                if (_ITEM4.LVL >= 1 && _ITEM4.CATEGORY !== 'extra') {
                  $(_levelText2).text(_ITEM4.LVL);
                }

                $(_equipmentSlot2).attr("data-type", _ITEM4.CATEGORY).attr("data-drone-id", _DRONE.ID).attr("data-item-id", _ITEM4.ID).attr("data-item-name", _ITEM4.ITEM_NAME.toLowerCase()).addClass("equiped-item").append($(_levelText2));
              } else {
                $(_equipmentSlot2).addClass("equipment-slot");
              } //APPEND SLOT


              $(droneEquipmentSlots).append(_equipmentSlot2);
            } //APPEND ALL SLOTS
            ...
            $(droneContainer).append(droneULevel); //CREATE DESIGN SLOT

            var designSlot = $('<div>').html('c');

            if (_DRONE.DRONE_DESIGN !== null && _DRONE.DRONE_DESIGN !== undefined) {
              $(designSlot).attr("data-type", _DRONE.DRONE_DESIGN.CATEGORY.toLowerCase()).attr("data-drone-id", _DRONE.ID).attr("data-item-id", _DRONE.DRONE_DESIGN.ID).attr("data-item-name", _DRONE.DRONE_DESIGN.ITEM_NAME.toLowerCase()).addClass("equiped-item");
            } else {
            ...
            $(droneContainer).append(droneDesignSlots); //APPEND DRONE TO HANGAR
        ...
        for (var _ITEM5 in ITEMS_UNUSED) {
          _ITEM5 = ITEMS_UNUSED[_ITEM5]; // filter select

          if (equipment.filter !== null) {
            if (equipment.filter.indexOf(_ITEM5.CATEGORY) === -1) {
              continue;
            }
          ...
          if (equipment.display === 'drone') {
            if (_ITEM5.CATEGORY === 'drone_formation' || _ITEM5.ATTRIBUTES.SPEED !== null || _ITEM5.CATEGORY === 'heavy' || _ITEM5.CATEGORY === 'extra' || _ITEM5.CATEGORY === 'gear' || _ITEM5.CATEGORY === 'protocols') {
              continue;
            }
          }

          if (equipment.display === 'ship') {
            if (_ITEM5.CATEGORY === 'drone_formation' || _ITEM5.CATEGORY === 'gear' || _ITEM5.CATEGORY === 'protocols' || _ITEM5.CATEGORY === 'drone_design') {
              continue;
            }
          ...
          if (equipment.display === 'pet') {
            if (_ITEM5.CATEGORY === 'drone_formation' || _ITEM5.ATTRIBUTES.SPEED !== null || _ITEM5.CATEGORY === 'heavy' || _ITEM5.CATEGORY === 'extra' || _ITEM5.CATEGORY === 'drone_design') {
              continue;
            }
          }

          var inventoryContainer = $(".ship-inventory-content > div > div").first(),
              itemDiv = $('<div>').addClass("inventory-item").attr("data-type", _ITEM5.CATEGORY.toLowerCase()).attr("data-item-id", _ITEM5.ID).attr("data-item-credits", _ITEM5.SELLING_CREDITS).attr("data-item-name", _ITEM5.ITEM_NAME.toLocaleLowerCase());

          if (_ITEM5.LVL >= 1 && _ITEM5.CATEGORY !== 'extra' && _ITEM5.ATTRIBUTES.SPEED === null) {
            $(itemDiv).append($('<span>').text(_ITEM5.LVL));
          }
    ...
    key: "switchDisplay",
    value: function switchDisplay(config) {
      ...
      $(".loading-screen").show();
      var Tabs = $("#config-tabs-content > ul.nav.nav-tabs > li").removeClass("active");
      ...
      $('#hangar-info-container .player-ship-view .' + equipment.display + '-box').addClass('active'); //SWITCH TABS

      $(Tabs[0]).find("a").attr("onclick", "equipment.switchDisplay(1,'" + equipment.display + "')");
      $(Tabs[1]).find("a").attr("onclick", "equipment.switchDisplay(2, '" + equipment.display + "')"); //RENDER EQUIPMENT
    ...
    value: function activateEquipment() {
      var selected = []; //MAKE INVETORY ITEMS DRAGABLE
          ...
          if (categories.includes($(ui.draggable).data('type'))) {
            ...
            if (selected.length > 1) {
              for (var _i2 = 0; _i2 < selected.length; _i2++) {
                ITEM_NAME = $(selected[_i2]).data("item-name");
                ITEM_CATEGORY = $(selected[_i2]).data("type");
                ITEM_ID = $(selected[_i2]).data("item-id");
        ...
        drop: function drop(event, ui) {
          ...
          if (selected.length > 1) {
            for (var _i3 = 0; _i3 < selected.length; _i3++) {
              ITEM_NAME = $(selected[_i3]).data("item-name");
              ITEM_CATEGORY = $(selected[_i3]).data("type");
              ITEM_ID = $(selected[_i3]).data("item-id");
              ITEM = {
                'NAME': ITEM_NAME,
                'CATEGORY': ITEM_CATEGORY,
                'ID': ITEM_ID
      ...
      $(".ship-inventory-content").selectable({
      ...
      $('.inventory-item').contextmenu(function (event) {
        var selected = $(".ui-selected:not(.ui-draggable-dragging)").each(function () {});
      ...
      $('.drone-container').contextmenu(function (event) {
        event.preventDefault();
        var droneID = parseInt($(this).data('drone-id'));
    ...
    key: "sellDrone",
    value: function sellDrone() {
      var data = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
      var droneID = arguments.length > 1 ? arguments[1] : undefined;
    ...
    */
    ...
    key: "resetCurrentConfig",
    value: function resetCurrentConfig() {
      var data = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
    ...
    key: "switchDesign",
    value: function switchDesign() {
      var data = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
      var designID = arguments.length > 1 ? arguments[1] : undefined;

      if (data !== null) {
        if (!data.error) {
          //SET NEW SHIPIMAGE
          var shipIMAGE = data.NEW_SHIP;
    ...
    key: "switchHangar",
    value: function switchHangar() {
      var data = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
      var hangarID = arguments.length > 1 ? arguments[1] : undefined;
    ...
    key: "switchFilter",
    value: function switchFilter(filterID) {
      var filter = '';
    ...
    key: "moveItemsTo",
    value: function moveItemsTo() {
      var data = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
      var ITEMS = arguments.length > 1 ? arguments[1] : undefined;
      var TO = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'inv';
...
/*#__PURE__*/
function () {
  function galaxygate(USER_ID, PLAYER_ID, SERVER_IP) {
    _classCallCheck(this, galaxygate);
  ...
  _createClass(galaxygate, null, [{
    ...
    value: function displayGate() {
      ...
      if (data === null) {
        ...
        var parts = parseInt(data['PARTS']);
        var total = parseInt(data['TOTAL']);
        console.log(data);
        var prepared = data['PREPARED'];
        var currentWave = $('#current-wave');
        var livesLeft = $('#lives-left');
        var gateSmoke = $('#gate-smoke > img');
        var prepareGate = $('#prepare-gate');
        currentWave.text('');
        livesLeft.text('');
        prepareGate.hide();
        gateSmoke.hide();
    ...
    value: function click() {
      ...
      if (data === null) {
        ...
        galaxygate.MULTIPLIER = data['MULTIPLIER'];
        galaxygate.updateMultiplier();
        if (galaxygate.MULTIPLY) galaxygate.toggleMultiply();
        this.displayGate();
      }
    }
  }, {
...
/*#__PURE__*/
function () {
  function modals(USER_ID, PLAYER_ID, SERVER_IP) {
...
/*#__PURE__*/
function () {
  function payment(USER_ID, PLAYER_ID) {
  ...
  _createClass(payment, null, [{
    ...
    value: function render() {
      $('#uridium-card .buy-now').click(function () {
        $("#gateway").modal();
        var finalPrice = parseFloat($(this).data('total'));
      ...
      $('#uridium-card .price').focus(function () {
        ...
        var finalPrice = parseFloat($(this).html());
...
/*#__PURE__*/
function () {
  function shop(USER_ID, PLAYER_ID, SERVER_IP, PET, ADMIN) {
    ...
    sendQuietRequest('shop', 'category_ids', {
      category: 'ammo'
    }, function (ammo_data) {
  ...
  _createClass(shop, null, [{
    ...
    value: function render() {
      ...
      if (shop.data === null && data === null) {
        ...
        ITEMS.forEach(function (ITEM, INDEX) {
          if (ITEM.SHOW_FUEL || ITEM.SHOW_CATS || ITEM.HAS_PET || ITEM.HAS_MAX_IRIS || ITEM.hasShip || ITEM.hasDesign || ITEM.hasFormation) {} else {
            var ItemDIV = $('<div>').addClass('item').attr('data-item-id', INDEX),
                ItemIMG = $('<div>').addClass('item-image'),
                ItemPRICE = $('<span>').addClass('item-price');
            var IMG_URL = ITEM.IMAGE_URL;
            $(ItemIMG).attr('style', 'background-image: url("' + IMG_URL + '")');
            var CURRENCY = ITEM.CURRENCY === 1 ? "C" : "U";
    ...
    value: function activateShop() {
      ...
      $('.amount-select .add-button').click(function (event) {
        var QTY = $('.amount-select .item-quantity');
        var TO_ADD = parseInt($(this).data('add')),
            CURRENT = parseInt(QTY.val());
        QTY.val(TO_ADD + CURRENT).change();
      });
      $('.amount-select .item-quantity').on('change', function () {
        var ITEM_ID = $('.single-item .single-item-content .single-item-buy-menu .buy-btn').data('item-id'),
            ITEM = shop.data[ITEM_ID],
      ...
      $('.single-item .single-item-content .single-item-buy-menu .buy-btn').click(function (event) {
        var ITEM_ID = $(this).data('item-id');
        var ITEM_LVL = $('.level-select .selected').data('level');
      ...
      $('.single-item .single-item-content .single-item-buy-menu .lvl-btn').click(function (event) {
        ...
        $(this).addClass('selected');
        $('.single-item .single-item-content .single-item-buy-menu .item-price').text((ITEM.PRICE * LVL).format(2, 0, ',', '.') + CURRENCY);
      });
    }
    /**
    ...
    key: "buyItem",
    value: function buyItem(ITEM_ID, AMOUNT, LEVEL) {
      var params = {
        "CATEGORY": shop.category,
        "ITEM_ID": ITEM_ID,
        "AMOUNT": AMOUNT,
    ...
    key: "buyCallback",
    value: function buyCallback(data) {
      if (data.status === "success") {
        swal("Success!", data.message, "success");
        var action = shop.packets[shop.category];

        if (shop.category === 'ammo' || shop.category === 'pet' || shop.category === 'pet_fuel') {
    ...
    key: "sendPacket",
    value: function sendPacket(action) {
...
var SkylabTimer = function SkylabTimer() {};
...
SkylabTimer.prototype = {
  ...
  barMinWidth: 20,
  barMaxWidth: 272,
  init: function init(divId, currentTime, start, end) {
    ...
    this.timeDiv = document.getElementById('progress_timer_' + divId); // draw timer...
...
/*#__PURE__*/
function () {
  function skylab(USER_ID, PLAYER_ID) {
    _classCallCheck(this, skylab);
  ...
  _createClass(skylab, null, [{
    ...
    key: "closeAllModules",
    value: function closeAllModules() {
      $('.module_large').hide();
      skylab.hideShadow();
    }
  }, {
    ...
    value: function showModule(moduleName) {
      ...
      module.show(); // automatically show upgrade if running

      if ($('#module_' + moduleName + '_small').hasClass('upgrading')) {
        console.log('UPGRADING....!');
        module.find('.skylab_module_tabs').children().eq(module.find('.progress_timer').closest('.tabContent').index()).click();
      }
    ...
    key: "deactivateCollector",
    value: function deactivateCollector(collectorName) {
      var boxName = collectorName.charAt(0).toLowerCase() + collectorName.slice(1);
      var box = $('#module_' + boxName + '_small');
      var topLeft = box.find('#corner_small_top_left_active').attr('id', 'corner_small_top_left_inactive');
      ...
      var bottomLeft = box.find('#corner_small_bottom_left_active').attr('id', 'corner_small_bottom_left_inactive');
    ...
    key: "unreadyCollector",
    value: function unreadyCollector(collectorName) {
      var boxName = collectorName.charAt(0).toLowerCase() + collectorName.slice(1);
      var box = $('#module_' + boxName + '_small');
    ...
    key: "changeToTab",
    value: function changeToTab(tabID, container) {
    ...
    key: "checkTransportTime",
    value: function checkTransportTime(value, isPremium) {
      var countElements = 0;
    ...
    key: "switchTransportDirection",
    value: function switchTransportDirection(newDir) {
      var lab = $('#to_skylab');
      var ship = $('#to_ship');
      var direction = $('#direction_but');
    ...
    key: "buySkylabRobot",
    value: function buySkylabRobot(data, robotId, collector) {
      var _this = this;
      ...
      if (data === null) {
        ...
        var msgType = data['IS_ERROR'] === 1 ? 'error' : 'success';
        swal(msg, '', msgType).then(function () {
          _this.reload();
        });
      }
    }
    ...
    key: "toggle",
    value: function toggle(data, collector, state) {
      var _this2 = this;
    ...
    key: "instantUpgradePopup",
    value: function instantUpgradePopup() {
      swal('Disabled instant upgrades.', '', 'warning');
    }
  }, {
    key: "buildUpgrade",
    value: function buildUpgrade(data, module) {
      var _this3 = this;
    ...
    key: "chooseUpgradeOption",
    value: function chooseUpgradeOption(option, containerKey) {
      var showAcceptButton = true;
      var infoBox = $('#module_infobox_' + containerKey); // set visual active state to the currently clicked container

      infoBox.find('.option_state').removeClass('upgrade_option_active');
      infoBox.find('.upgrade_option_' + option + ' .option_state').addClass('upgrade_option_active'); // hide the ok button

      infoBox.find('.upgrade_option_confirm').hide();
      var textId = '#upgrade_text_' + option; // check if we have a video available
      ...
      infoBox.find(textId + '_' + containerKey).show(); // set the clicked option as chosen

      infoBox.find('#upgrade_option_' + containerKey).val(option); // unbind the click event from the ok button to avoid duplication and/or exploits

      infoBox.find('.upgrade_option_confirm').unbind('click'); // show ok button and bind the click event
    ...
    key: "instantUpgrade",
    value: function instantUpgrade(containerKey) {
      console.log('instantUpgrade');
    }
  }, {
    key: "cancelUpgrade",
    value: function cancelUpgrade(containerKey) {
      console.log('cancelUpgrade');
    }
  }, {
    key: "showVideo",
    value: function showVideo(containerKey) {
...
__webpack_require__(/*! C:\xampp\htdocs\univ3rse\wwwroot_laravel\resources\js\app.js */"./resources/js/app.js");
module.exports = __webpack_require__(/*! C:\xampp\htdocs\univ3rse\wwwroot_laravel\resources\sass\app.scss */"./resources/sass/app.scss");
